...
// argv: pointer to null-terminated arrary of pointers to strings.
...
spawn(const char *prog, const char **argv)
{
	...
	//
	//		- Read the ELF header, as we have before, and sanity check its
	//		  magic number. (Check out our load_icode!)
	//
	...
	//	  from the executable file - we must clear the rest to zero.
	...
	//	  (We can avoid calling memset(), if page_alloc() returns zeroed
	//	  pages already.)
	//		Then insert the page mapping into the child.
	//		Look at init_stack() for inspiration.
	//		Be sure understand why we can't use read_map() here.
	//
	//	  Note: None of the segment address or lengths above
	//	  are guaranteed to be page-aligned, so we must deal with
	//	  these non-page-aligned values appropriately.
	//	  The ELF linker does, however, guarantee that no two segments
	//	  will overlap on the same page; and it guarantee that
...
/*	if ((r = copy_shared_pages(child)) < 0) {
	...
	if ((r = sys_env_set_status(child, ENV_RUNNABLE)) < 0) {
		panic("In spawn sys_env_set_status failed: %e", r);
	}
...
init_stack(envid_t child, const char **argv, uintptr_t *init_esp)
{
	...
	if ((r = sys_page_alloc(0, (void*) UTEMP, PTE_P | PTE_U | PTE_W))) {
		return r;
	}

	// * Initial 'argv_store[i]' to point to argument string i,
	//	for all 0 <= i < argc.
